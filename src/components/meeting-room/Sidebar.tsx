...
export function Sidebar({
  ...
  return (
    <aside className="flex min-h-0 flex-col rounded-[32px] border border-gray-200 bg-white shadow-sm">
      <div className="border-b border-gray-200 p-3">
        <div className="grid grid-cols-4 gap-2 rounded-[24px] bg-gray-100 p-1">
          {tabs.map((tab) => {
            ...
            return (
              ...
              >
                <span>{tab.label}</span>
                <span className={["ml-2 text-xs", isActive ? "text-gray-500" : "text-gray-400"].join(" ")}>
                  {counts[tab.id]}
                </span>
              </button>
            )
